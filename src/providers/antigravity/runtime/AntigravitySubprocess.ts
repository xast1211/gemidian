...
export interface AntigravitySubprocessLaunchSpec {
  args: string[];
  command: string;
  cwd: string;
...
export class AntigravitySubprocess {
  ...
  constructor(private readonly launchSpec: AntigravitySubprocessLaunchSpec) {}
  ...
  private requireProc(): ChildProcessWithoutNullStreams {
    if (!this.proc) {
      throw new Error('Antigravity subprocess is not started');
    }
    return this.proc;
  }
  ...
  start(): void {
    ...
    proc.on('exit', (code, signal) => {
      const exitError = this.closeError ?? (
        code === 0 && signal === null
          ? undefined
          : new Error(`Antigravity subprocess exited (${formatExit(code, signal)})`)
      );
